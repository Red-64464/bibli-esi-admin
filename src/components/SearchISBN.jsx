...
function BookResult({ book, source, onConfirm }) {
  return <div className="flex min-w-0 flex-col gap-4 rounded-lg border border-biblio-accent/30 bg-white/5 p-4 sm:flex-row">{book.couverture_url && <img src={book.couverture_url} alt={book.titre} className="h-36 w-24 shrink-0 rounded object-contain" />}<div className="min-w-0 flex-1 space-y-1"><h3 className="break-words font-semibold text-biblio-text">{book.titre}</h3><p className="break-words text-sm text-biblio-muted">{book.auteur || "Auteur inconnu"}</p>{book.editeur && <p className="break-words text-xs text-biblio-muted">Éditeur : {book.editeur}</p>}{book.annee && <p className="text-xs text-biblio-muted">Année : {book.annee}</p>}{book.isbn && <p className="break-all font-mono text-xs text-biblio-muted">ISBN : {book.isbn}</p>}{source === "catalogue" && <p className="text-xs text-biblio-success">Ce livre est déjà dans votre catalogue.</p>}</div><button onClick={onConfirm} disabled={source === "catalogue"} className="flex w-full items-center justify-center rounded-lg bg-biblio-success px-5 py-2.5 font-medium text-white disabled:opacity-50 sm:w-auto sm:self-center"><CheckCircle className="mr-2 inline h-4 w-4" />{source === "catalogue" ? "Déjà ajouté" : "Ajouter"}</button></div>;
}
